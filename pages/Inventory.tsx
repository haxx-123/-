...
import { Search, ChevronDown, ChevronRight, Plus, Package, FileText, Edit2, X, Save, ArrowRightLeft, Lock, Info, ScanLine, Filter, ChevronLeft } from 'lucide-react';
          ...
          <div className="space-y-4">
             ...
             <div className="flex gap-4">
               ...
               <div className="w-1/3">
                 ...
                 >
                   <option value="big">整 ({batch.unitBig})</option>
                   <option value="small">散 ({batch.unitSmall})</option>
                 </select>
               </div>
             </div>
          </div>
...
const AdjustModal = ({ target, type, onClose }: { target: Product | Batch; type: 'product' | 'batch'; onClose: () => void }) => {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white dark:bg-gray-800 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
          ...
          {isBatch && batchTarget ? (
            <>
              <div>
                ...
                <input type="text" defaultValue={batchTarget.batchNumber} className="input-std" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">有效期</label>
                <input type="date" defaultValue={batchTarget.expiryDate} className="input-std" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                 <div>
                   <label className="block text-sm font-medium mb-1 text-blue-600 font-bold">库存 (整-{batchTarget.unitBig})</label>
                   <input type="number" defaultValue={batchTarget.quantityBig} className="input-std font-bold" />
                 </div>
                 <div>
                   <label className="block text-sm font-medium mb-1 text-green-600 font-bold">库存 (散-{batchTarget.unitSmall})</label>
                   <input type="number" defaultValue={batchTarget.quantitySmall} className="input-std font-bold" />
                 </div>
              </div>
              <div>
                 <label className="block text-sm font-medium mb-1">换算率 (1整 = ?散)</label>
                 <input type="number" defaultValue={batchTarget.conversionRate} className="input-std" />
              </div>
            </>
          ) : productTarget ? (
            <>
              <div className="flex gap-4 mb-2">
                 {productTarget.image_url && (
                    <img src={productTarget.image_url} className="w-24 h-24 rounded-xl object-cover border" alt="Product" />
                 )}
                 <div className="flex-1">
                    <label className="block text-sm font-medium mb-1">商品图片URL</label>
                    <input type="text" defaultValue={productTarget.image_url} className="input-std text-sm" placeholder="https://..." />
                 </div>
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">商品名称</label>
                <input type="text" defaultValue={productTarget.name} className="input-std" />
              </div>
              <div>
                 <label className="block text-sm font-medium mb-1">SKU</label>
                 <input type="text" defaultValue={productTarget.sku} className="input-std" />
              </div>
              <div>
                 <label className="block text-sm font-medium mb-1">分类</label>
                 <input type="text" defaultValue={productTarget.category} className="input-std" />
              </div>
              <div>
                 <label className="block text-sm font-medium mb-1">备注 (Notes)</label>
                 <textarea defaultValue={productTarget.notes} className="input-std h-24" />
              </div>
            </>
          ) : null}
        ...
        <div className="p-4 border-t dark:border-gray-700 bg-gray-50 dark:bg-gray-900 flex gap-3">
           ...
           <button onClick={() => { alert('保存成功'); onClose(); }} className="flex-1 py-3 rounded-xl bg-blue-600 text-white font-bold hover:bg-blue-700 shadow-lg shadow-blue-600/20 flex items-center justify-center gap-2">
...
const ProductDetailModal = ({ product, onClose }: { product: Product; onClose: () => void }) => (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black bg-opacity-80 backdrop-blur-sm animate-fade-in" onClick={onClose}>
        <div className="max-w-xl w-full bg-transparent p-4" onClick={e => e.stopPropagation()}>
            <img src={product.image_url} alt={product.name} className="w-full rounded-2xl shadow-2xl mb-4" />
...
const Inventory = () => {
  const { user, currentStore, setPageActions, isMobile } = useApp();
  ...
  const [adjustModal, setAdjustModal] = useState<{ open: boolean; target?: any; type?: 'product' | 'batch' }>({ open: false });
  ...
  const renderMobileView = () => (
    <div className="space-y-4">
        {filteredProducts.map(product => {
            ...
            const unitBig = product.batches[0]?.unitBig || '盒';
            const unitSmall = product.batches[0]?.unitSmall || '粒';
                                    ...
                                    <div className="flex justify-between items-center">
                                        ...
                                        {canEdit && (
                                            <div className="flex gap-2">
                                                ...
                                                <button onClick={() => setAdjustModal({ open: true, target: batch, type: 'batch' })} className="text-xs text-gray-600 bg-gray-200 px-2 py-1 rounded">调整</button>
                                            </div>
                                        )}
                                    </div>
                                </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 animate-fade-in-up">
        ...
        <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
           ...
           <div className="relative flex-1 min-w-[250px]">
              ...
              <input 
                type="text" 
                placeholder="搜索: 拼音/汉字 (如 tms 查土霉素)" 
                value={searchTerm}
                        ...
                        <tr className="bg-gray-50 dark:bg-gray-900/30 inset-shadow">
                            ...
                            >
                                <div className="grid grid-cols-8 gap-4 text-xs font-medium text-gray-500 px-4 py-2 border-b dark:border-gray-700 uppercase">
                                    <div className="col-span-2">批号</div>
                                ...
                                <div key={batch.id} className="grid grid-cols-8 gap-4 items-center px-4 py-3 bg-white dark:bg-gray-800 rounded-lg border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-all hover:-translate-y-0.5">
                                    <div className="col-span-2 font-mono text-blue-600 dark:text-blue-400">{batch.batchNumber}</div>
                                    ...
                                    <div className="col-span-2 flex justify-end gap-2">
                                        ...
                                        <>
                                            ...
                                            <button 
                                            onClick={() => setAdjustModal({ open: true, target: batch, type: 'batch' })}
